...
export default function GameBoard() {
  ...
  const startAIMove = useCallback(async () => {
    ...
    let target = -1;
    const triples = [[0,1,2],[2,3,4],[4,5,6],[6,7,0],[8,9,10],[10,11,12],[12,13,14],[14,15,8],[16,17,18],[18,19,20],[20,21,22],[22,23,16],[1,9,17],[3,11,19],[5,13,21],[7,15,23]];
    
    for (const idx of empty) {
      const testBoard = [...board];
      testBoard[idx] = 'black';
      if (triples.some(t => t.every(i => testBoard[i] === 'black'))) {
        target = idx; break;
      }
    }
    ...
    if (target === -1) {
      for (const idx of empty) {
        const testBoard = [...board];
        testBoard[idx] = 'white';
        if (triples.some(t => t.every(i => testBoard[i] === 'white'))) {
          target = idx; break;
        }
      }
    }
